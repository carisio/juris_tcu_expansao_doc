...
def plot_metricas(grupo=1, calcular_percentual=True):
    if grupo == 1:
    ...
    for idx_metrica, label_metrica in enumerate(label_metricas):
        ...
        sep_grupos = 1
        x_expansao_doc = np.arange(len(ordem_expansao_doc))
        x_semantico = np.arange(len(ordem_semantico)) + len(ordem_expansao_doc) + sep_grupos  # Separação entre os grupos
        if not calcular_percentual: # Se não for percentual, aí tem que adicionar o baseline
            x_baseline = [0]
            x_expansao_doc = x_expansao_doc + len(x_baseline) + sep_grupos
            x_semantico = x_semantico + len(x_baseline) + sep_grupos
    
        # Barras
        if not calcular_percentual: # Se não for percentual, aí tem que adicionar o baseline
            ax.bar(x_baseline, [baseline_value], color=colors["baseline"], label="baseline")
        barras_exp_doc = ax.bar(x_expansao_doc, expansao_doc_values, color=colors["expansao_doc"], label="expansao_doc")
        barras_semantico = ax.bar(x_semantico, semantico_values, color=colors["semantico"], label="semantico")
        ...
        if not calcular_percentual:
            for barra in barras_exp_doc + barras_semantico:
                ...
                ax.text(barra.get_x() + barra.get_width() / 2, altura/2,  # Posição do texto
                        valor_para_exibir,  # O texto a ser exibido
                        ha='center', va='bottom', fontsize=12, rotation=90)  # Alinhamento e tamanho da fonte
            ax.set_ylim(0, 100)
    
        # Rótulos no eixo x e y e título
        if calcular_percentual:
            # Eixo x
            ax.set_xticks(list(x_expansao_doc) + list(x_semantico))
            ax.set_xticklabels(ordem_expansao_doc + ordem_semantico, rotation=90)
            # Eixo y
            ax.set_ylabel("Percentage relative to the baseline")
            # Título
            ax.set_title(label_metrica)
        else:
            # Eixo x
            ax.set_xticks(list(x_baseline) + list(x_expansao_doc) + list(x_semantico))
            ax.set_xticklabels(["Baseline"] + ordem_expansao_doc + ordem_semantico, rotation=90)
            # Eixo y
            ax.set_ylabel(f"{label_metrica}")
            # Título
            #ax.set_title(label_metrica)
        ...
        if calcular_percentual:
            ...
            ax.axhline(baseline_value, color="black", linestyle="--", linewidth=0.8)
...
plot_metricas(grupo=1, calcular_percentual=False)
